...
func update_object(object, position_change):
	...
	#adjust position on planet based on rotation
	var vector = object.global_position - global_position
	vector = vector.rotated(rotation_axis, (2*PI)/(rot_period))
	object.velocity = (vector - (object.global_position - global_position))
